...
class UR5RobotiqPybulletController:
    ...
    def __init__(self,
                 urdf='assets/figures-ur5/ur5_robotiq.urdf',
    ...
    def plan_arm_joint_values_simple(self, goal_joint_values, start_joint_values=None, duration=None):
        ...
        waypoints = np.vstack(
            (waypoints, np.repeat(waypoints[-1, None], 240, axis=0)))
        return waypoints
    ...
    def move_arm_to(self,
                    ...
                    duration=2):
        if simple:
            plan = self.plan_arm_joint_values_simple(
                target_jv, duration=duration)
        else:
