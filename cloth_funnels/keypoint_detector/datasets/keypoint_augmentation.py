...
def build_augmentation_pipeline(cfg: dict, height=None, width=None, apply_prob=0.5):
    ...
    if cfg.get("scale", 0.4) > 0:
        scale_jitter_lo = cfg.get('scale_jitter_lo', 0.75)
        scale_jitter_high = cfg.get('scale_jitter_lo', 1.25)
        pipeline.add(
            iaa.Sometimes(
                cfg.get("scale", 0.2),
                iaa.Affine(scale={'x':(scale_jitter_lo, scale_jitter_high), "y":(scale_jitter_lo, scale_jitter_high)}),
            )
        )
    ...
    if cfg.get("rotation", None) > 0:
        pipeline.add(
            iaa.Sometimes(
                ...
                iaa.Affine(rotate=(-cfg.get("rotation", None), cfg.get("rotation", None)), mode='edge'),
            )
        )
