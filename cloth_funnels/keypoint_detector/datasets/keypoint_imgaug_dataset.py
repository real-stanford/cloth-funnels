...
from .keypoint_augmentation import (
...
class KeypointImgaugDataset(torch.utils.data.Dataset):
    def __init__(self, 
        ...
        **kwargs):
        ...
        self.original_img_shape = sample_img.shape[:2]

        sample_img = cv2.resize(sample_img, (target_img_size, target_img_size))
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        def scale_pixel(pix, input_dim=480, target_dim=self.target_img_size, inverse=False):
                    return int(pix * target_dim/input_dim)

        keypoints = KeypointsOnImage([Keypoint(x=scale_pixel(X), y=scale_pixel(Y)) for X,Y in keypoint], shape=img_norm.shape)

        img_aug, kp_aug = imgaug_pipeline(image=img_norm, keypoints=keypoints)
        kp_aug = np.array([[k.x, k.y] for k in kp_aug], dtype=np.float32)

        target_scoremaps = []

        for coord in kp_aug:
            target_scoremaps.append(get_gaussian_scoremap(
                img_aug.shape[:2], 
                keypoint=coord,
                sigma=sigma))
        
        target_scoremaps = np.stack(target_scoremaps)

        data = {
            'input_image': np.moveaxis(img_aug, -1, 0),
            'target_scoremap': target_scoremaps,
...
def get_data_df(data_dir, json_name='labels.json', img_format='jpg'):
    ...
    for json_path in Path(data_dir).glob("*"+json_name):
        ...
        for key in valid_keys:
            ...
            rows_dict[key_tuple] = {
                'keypoint': np.array(this_labels_dict[key]),
...
if __name__ == '__main__':
    ...
    cfg = OmegaConf.load(to_absolute_path('../config/train.yaml'))
    m = KeypointImgaugDataModule(**cfg.datamodule)
    m.prepare_data()
    train_dataset = m.get_dataset('train')
    train_dataset[0]
    # dataset = KeypointImgaugDataset()
# %%
